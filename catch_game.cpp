...
void updateCatchingGame() {
  ...
  unsigned long dt = lastUpdateTime ? (now - lastUpdateTime) : 0;
  lastUpdateTime = now;
  float dtSec = dt / 1000.0f;  // convert ms → seconds

  gY = myIMU.readFloatAccelX();
  // Map the IMU value to a movement speed.
  float moveSpeed = gY * 60;
  ...
  if(now - lastFallingSpawnTime > (unsigned long)fallingSpawnInterval) {
    for (int i = 0; i < MAX_FALLING_OBJECTS; i++) {
      if (!fallingObjects[i].active) {
        ...
        fallingObjects[i].speed = random_int(20, 40);
...
static void cleanupCatchingGame() {
  ...
  // 1) Destroy falling objects
  for (int i = 0; i < MAX_FALLING_OBJECTS; i++) {
    if (fallingObjects[i].sprite) {
      fallingObjects[i].sprite->destroy();
      delete fallingObjects[i].sprite;
      fallingObjects[i].sprite = nullptr;
    }
    fallingObjects[i].active = false;
  }

  // 2) Destroy caught‐stack
  if (caughtStack) {
    caughtStack->destroy();
    delete caughtStack;
    caughtStack = nullptr;
  ...
  caughtStackCount = 0;
  memset(caughtTypes, 0, sizeof(caughtTypes));

  // 3) Destroy the player on the game screen
  myStack.destroy();

  // 4) Switch back to the original screen *before* deleting gameScreen
  lv_scr_load(mainScreen);
  inCatchingGame = false;

  // 5) Now delete the game screen container itself
  lv_obj_del(gameScreen);
  gameScreen = nullptr;

  // 6) Re-create the player on mainScreen
